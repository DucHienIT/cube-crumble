...
Runtime-bootstrap architecture: the scene contains only a camera and one `GameBootstrap` component; everything else (EventSystem, audio, UI, game state) is built in code at Play time. All sprites and audio are generated procedurally — zero image/sound assets.
...
- `Scenes/Main.unity` — the only scene; first in Build Settings.
...
- Win = all balls delivered (total container capacity == total balls, enforced when generating levels); stars by remaining time (≥50% → 3, ≥25% → 2, else 1).
